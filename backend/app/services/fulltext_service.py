...
class FullTextService:
    ...
    def download_pdf(self, arxiv_id: str, save_dir="/tmp/arxiv_pdfs") -> str:
        """
        Download arXiv PDF and return local path. ( /tmp/arxiv_pdfs/{arxiv_id}.pdf )
        # ls -lh /tmp/arxiv_pdfs       使用这个命令可以查看目录下 downloaded 的 PDF 文件
        """
        ...
        # Download the PDF with streaming
        response = requests.get(url, stream=True)
        if response.status_code != 200:
            raise Exception(f"Failed to download PDF: {url}")

        # Save the PDF to local file and write in chunks
